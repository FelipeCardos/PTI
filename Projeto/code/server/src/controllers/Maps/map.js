...
async function getCoordinatesFromUserAndProduct(userId, productId) {
    let result = [];
      ...
      console.log(userAddress.street + ', ' + userAddress.postal_code + ', ' + userAddress.state + ', ' + userAddress.country);

      await axios.get(url, {
        params: {
          text: userAddress.street + ', ' + userAddress.postal_code + ', ' + userAddress.state + ', ' + userAddress.country,
          format: 'json',
          apiKey: apiKey,
        }
      })
      ...
      console.log(producerAddress.street + ', ' + producerAddress.postal_code + ', ' + producerAddress.state + ', ' + producerAddress.country);
      await axios.get(url, {
        params: {
          text: producerAddress.street + ', ' + producerAddress.postal_code + ', ' + producerAddress.state + ', ' + producerAddress.country,
          format: 'json',
          apiKey: apiKey,  
        }
      })
...
module.exports = {getCoordinatesFromUserAndProduct, calculatedistance};