  ...
  useEffect(() => {
    ...
    async function getCartlineData(cart_id, product_id) {
      const response = await axios.get(
        `https://yourlocalshop.pt:3000/api/v1/carts/${cart_id}/cartlines`
      );
    ...
    async function getProductionUnitData(id) {
      let response = await axios.get(
        `https://yourlocalshop.pt:3000/api/v1/productionUnits/${id}`
      );
      const productionUnitAddress = await axios.get(
        `https://yourlocalshop.pt:3000/api/v1/users/${response.data.productionUnit.producer_id}/productionUnits/${id}/address`
      );
    ...
    async function getVehiclesFromProductionUnit(
      ...
      const response = await axios.get(
        `https://yourlocalshop.pt:3000/api/v1/users/${producer_id}/productionUnits/${production_unit_id}/vehicles`
      );
    ...
    async function fetchData() {
      ...
      for (const vehicle of vehicles.vehicles) {
        if (
          await axios.get(
            `https://yourlocalshop.pt:3000/api/v1/vehicles/${vehicle.id}/availability`
          )
        ) {
